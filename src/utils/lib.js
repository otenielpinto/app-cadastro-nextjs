...
function obter_percentual(valor_venda, valor_nominal) {
  let result = 0;
...
function arrayToString(records) {
  const results = [];
  for (const record of records) {
    const keys = Object.keys(record);
    const novo = {};
...
function dateToBrString(data) {
  // converter para data no formato DD/MM/YYYY
  let data_formatada = "";
  if (data) {
    ...
    // Usar a data atual se dt_movto não estiver disponível
    const hoje = new Date();
